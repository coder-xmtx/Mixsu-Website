...
function onEnter() {
  ...
  if (!el) return;
  gsap.to(el, { x: 5, y: -2, opacity: 1, rotation: 0, duration: 0.3, ease: "power2.out" });
}

function onLeave() {
  const el = arrowEl.value;
  if (!el) return;
  gsap.to(el, { x: 0, y: 0, opacity: 0.6, duration: 0.3, ease: "power2.out" });
}
...
<template>
  <component :is="linkComponent" :to="to" :href="href" :target="isExternal ? '_blank' : undefined"
    ...
    <span class="slash-underline">{{ label }}</span>
