  ...
  >
    <NuxtLink :to="to" data-cursor="hover" class="flex h-full flex-col gap-5">
      ...
      <div class="relative aspect-[3/2] overflow-hidden rounded-2xl border border-line bg-surface">
        ...
        <div
          v-else
          class="grid size-full place-items-center font-display text-6xl font-extrabold text-stroke"
        >
          {{ title.slice(0, 1) }}
        </div>
        <!-- 分类角标 -->
      ...
      <div class="flex flex-1 flex-col gap-2">
        <div class="flex items-center justify-between gap-4">
          <h3 class="font-display text-xl font-bold tracking-tight text-text transition-colors duration-300 group-hover:text-accent">
            {{ title }}
          </h3>
