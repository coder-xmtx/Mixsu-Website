  ...
  <Reveal variant="up" :delay="(index ?? 0) * 0.12" :y-offset="48" class="h-full">
    ...
    >
      ...
      <div class="relative">
        ...
        <h3 class="mt-5 font-display text-4xl font-extrabold tracking-tight text-text transition-colors duration-300 group-hover:text-accent md:text-5xl">
          {{ meta.label }}
        </h3>
      ...
      <div class="relative mt-8 flex items-center justify-between">
        ...
        <span class="grid size-10 place-items-center rounded-full border border-line text-muted transition-all duration-500 group-hover:border-accent group-hover:bg-accent group-hover:text-bg">
          <UIcon name="lucide:arrow-right" class="size-4 transition-transform duration-300 group-hover:translate-x-0.5" />
        </span>
      </div>
    </NuxtLink>
  </Reveal>
