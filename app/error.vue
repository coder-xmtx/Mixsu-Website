...
<template>
  <div ref="el"
    ...
    <h1 data-error-fade
      class="mt-6 font-display text-[clamp(6rem,20vw,13rem)] font-extrabold leading-none tracking-tight text-stroke select-none">
      {{ is404 ? "404" : error.statusCode }}
    </h1>

    <p data-error-fade class="mt-6 max-w-md text-base leading-relaxed text-muted md:text-lg">
      {{ is404 ? "欢迎来到秘密之地 这里什么都没有" : (error.message || "出了点问题。") }}
    </p>
